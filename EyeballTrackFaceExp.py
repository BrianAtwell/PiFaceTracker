...
def applyExponential(point):
	global ExpS, ExpA
	ExpS.x=(point.x*ExpA.x)+(1-ExpA.x)*ExpS.x
	ExpS.y=(point.y*ExpA.y)+(1-ExpA.y)*ExpS.y
...
def updateServoController(facePoint):
	...
	applyExponential(facePoint)
	if faceDeltaTime < 0.5 and isPIDMode: #and PIDUpdateDeltaTime > 1.0:
		pidOutput.x = pidControllerXY[0].update(ExpS.x)
		pidOutput.y = pidControllerXY[1].update(CameraResolution[1]-ExpS.y)
		...
		if len(rects) >= 1:
			...
			if not nested.empty():
				for x1, y1, x2, y2 in rects:
					...
					draw_rects(roi, subrects, (255, 0, 0))
			lastFacePoint=clock()
